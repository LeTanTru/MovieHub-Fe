      ...
      onSuccess: (res) => {
        if (res.result) {
          invalidateQueries(
            ...
            [queryKeys.PLAYLIST_BY_MOVIES, movieId]
          );
          notify.success(
      ...
      <AnimatePresence mode='wait'>
        {opened && (
          ...
          >
            ...
            {playlist.length < MAX_PLAYLIST_COUNT && (
              <ButtonAddPlayList
                className='bg-golden-glow border-golden-glow hover:bg-golden-glow/80 rounded border-none text-black hover:text-black/80'
                text='Thêm mới'
                showText
              />
            )}
